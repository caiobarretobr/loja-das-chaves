...
function getVapidSubject() {
  return stripWrappingQuotes(process.env.VAPID_SUBJECT || 'mailto:loja-das-chaves@localhost');
}
...
async function notifySubscriptions(subscriptions, payload, removeInvalidSubscription) {
  const body = JSON.stringify({
    title: payload.title || 'Loja das Chaves',
    body: payload.body || '',
    tag: payload.tag || 'loja-chaves-alerta',
    url: payload.url || '/',
  });
